...
function VoidedLines({ lines, t }: { lines: RawData[]; t: (k: string) => string }) {
...
function SummaryLines({ lines, t }: { lines: RawData[]; t: (k: string) => string }) {
  if (lines.length === 0) return null
  return (
    <div className="flex flex-col gap-2">
...
function DespatchLines({ lines, t }: { lines: RawData[]; t: (k: string) => string }) {
  if (lines.length === 0) return null
  return (
    <div className="flex flex-col gap-2">
...
export function AdminDocCard({ detail }: AdminDocCardProps) {
  ...
  const invoiceType = detail.invoice_type
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {invoiceType === 'comunicacion_baja' && (
        <VoidedLines lines={(raw.voided_lines as RawData[] | undefined) ?? []} t={t} />
      )}

      {invoiceType === 'resumen_diario' && (
        <SummaryLines lines={(raw.summary_lines as RawData[] | undefined) ?? []} t={t} />
      )}

      {invoiceType === 'guia_remitente' && (
        <div className="flex flex-col gap-4">
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
            <Field label={t('detail.fieldRelatedInvoice')} value={str(raw.related_invoice)} mono />
