...
// Backend amounts arrive as DRF decimal strings. Format with the billing document
...
export const BILLING_DOCUMENT_STATUS_VARIANT: Record<string, BadgeVariant> = {
  received: 'secondary',
  parsing: 'outline',
  parsed: 'outline',
...
export const BILLING_DOCUMENT_TYPE_VARIANT: Record<string, BadgeVariant> = {
  factura: 'default',
  boleta: 'default',
...
export function statusLabel(status: string | null | undefined): string {
  if (!status) return '—'
  return BILLING_DOCUMENT_STATUS_LABEL[status] ?? status
}

export function statusVariant(status: string | null | undefined): BadgeVariant {
  if (!status) return 'outline'
  return BILLING_DOCUMENT_STATUS_VARIANT[status] ?? 'outline'
}

export function typeLabel(type: string | null | undefined): string {
  if (!type) return '—'
  return i18next.t(`type.${type}`, { ns: 'billing-documents', defaultValue: type })
}

export function typeVariant(type: string | null | undefined): BadgeVariant {
  if (!type) return 'outline'
  return BILLING_DOCUMENT_TYPE_VARIANT[type] ?? 'outline'
}
