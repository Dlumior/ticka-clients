...
import { SUNAT_ADMIN_TYPES } from '../../billing-documents.lib'
...
export function DetailBody({ detail }: DetailBodyProps) {
  ...
  const { timezone } = useBillingDocumentsContext()
  ...
  return (
    <>
      ...
      {(detail.billing_document_type === 'nota_credito' ||
        detail.billing_document_type === 'nota_debito') && (
        <div className="flex flex-col gap-3">
          <SectionLabel>{t('detail.sectionReference')}</SectionLabel>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
            <Field
              label={t('detail.fieldReferencedDocument')}
      ...
      {SUNAT_ADMIN_TYPES.has(detail.billing_document_type ?? '') ? (
        ...
        /* Regular billing document — amounts, supplier, line items */
        <>
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="flex flex-col gap-3">
