...
import { zBillingDocumentDetailOutputStatusEnum, zBillingDocumentTypeEnum } from '@repo/api-types'
...
export function BillingDocumentsToolbar() {
  const { t } = useTranslation('billing-documents')
  const { filters } = useBillingDocumentsContext()
    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      <Select value={filters.statusFilter} onValueChange={(v) => filters.onStatusChange(v ?? '')}>
        ...
        <SelectContent>
          ...
          {zBillingDocumentDetailOutputStatusEnum.options.map((s) => (
      ...
      <Select value={filters.typeFilter} onValueChange={(v) => filters.onTypeChange(v ?? '')}>
        ...
        <SelectContent>
          ...
          {zBillingDocumentTypeEnum.options.map((typeVal) => (
