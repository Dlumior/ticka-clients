...
import { zInboundEmailListOutput, zInboundAttachmentListOutput } from '@repo/api-types'
...
export const inboxEmailsQueryOptions = (workspaceId: string, params: InboxEmailsParams) =>
  queryOptions({
...
export const inboxAttachmentsQueryOptions = (workspaceId: string, emailId: string) =>
  queryOptions({
    queryKey: ['inbox-attachments', workspaceId, emailId],
    queryFn: ({ signal }) =>
      apiClient
        .get<InboundAttachment[]>(`/api/v1/ingestion/workspaces/${workspaceId}/attachments/`, {
          signal,
