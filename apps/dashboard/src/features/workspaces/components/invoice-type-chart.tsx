...
export function InvoiceTypeChart({ stats, className }: InvoiceTypeChartProps) {
  const { t } = useTranslation('workspaces')
  const byType = stats.invoices.by_type

  const data = useMemo(
    () =>
      TYPE_ORDER.map((type) => ({
        type,
        label: t(`metrics.type.${type}`),
        count: byType[type],
  ...
  const config: ChartConfig = { count: { label: t('metrics.invoices') } }
